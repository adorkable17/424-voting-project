...
def encrypt():
    ...
    with open("votekey.key","wb") as file:
        file.write(key)
    with open("voteiv.iv","wb") as file:
        file.write(iv)
...
def decrypt():
    try:
        with open("votekey.key","rb") as file:
            key=file.read()
        with open("voteiv.iv","rb") as f:
            iv=f.read()
        with open("myfile_EN.txt","rb") as f:
            cipht=f.read()
        cipher=AES.new(key, AES.MODE_CBC, iv)
        plaintext=unpad(cipher.decrypt(cipht),  AES.block_size)
        with open("myfile_DE.txt","wb") as f:
            f.write(plaintext)
    except(ValueError, KeyError):
        with open("myfile_DE.txt","wb") as f:
            f.write("Decryption failed")
...
def callBack():
    ...
    L2.pack(anchor = N)
    load = Image.open("Downloads/finalpicture.png")
